...
local placement_audit_log_service = {}
...
function placement_audit_log_service.append(dist_dir, level, message, ctx)
  if not is_non_empty_string(dist_dir) then
    return false, "dist_dir must be a non-empty string"
  end
  local line = placement_session_log_service.format(level or "INFO", message or "", ctx)
  local log_path = path_utils.join(dist_dir, "placement_audit.log")
...
return placement_audit_log_service
